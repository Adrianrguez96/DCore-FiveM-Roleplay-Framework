VehicleController = function()
    ...
    this.init = function()
        this.SpawnPlayerVehicles()
    end

    -- Event to spawn player vehicles 
    this.SpawnPlayerVehicles = function()
        RegisterNetEvent("Core:client:SpawnPlayerVehicles")
        AddEventHandler("Core:client:SpawnPlayerVehicles", function(playerVehicles)
            for _,vehicle in pairs(playerVehicles) do
                local position = json.decode(vehicle.position)
                this.VehicleSpawn(vehicle.model,position,true,function()
                end)
            end
        end)
    end

    -- Spawn vehicle in a specific position
    this.VehicleSpawn = function(model, coords, isnetworked, cb)
    ...
    this.GetVehiclePlate = function(vehicle)
        if vehicle == 0 then return end
        return Core.Utility.Trim(GetVehicleNumberPlateText(vehicle))
    end

    this.GetVehicleProperties = function(vehicle)
    ...
    this.SetVehicleProperties = function(vehicle, props)
        if DoesEntityExist(vehicle) then
            local colorPrimary, colorSecondary = GetVehicleColours(vehicle)
            local pearlescentColor, wheelColor = GetVehicleExtraColours(vehicle)
