local isInterfaceOpening = false
...
AddEventHandler('charactercreator:close', function(save)
    ...
    if save then
        ...
        registerData.skin = currentChar
        TriggerServerEvent('core:server:RegisterPlayer',registerData)
    else
...
RegisterNetEvent('charactercreator:open')
AddEventHandler('charactercreator:open', function(data,tabs)
...
AddEventHandler('charactercreator:getCurrentClothes', function(cb)
    local result = {}
...
AddEventHandler('charactercreator:updateClothes', function(data, save, updateOld, callback)
    UpdateClothes(data, updateOld)
    if save then
        TriggerServerEvent('charactercreator:save',registerData, currentChar)
    end
    if callback then
        callback()
    end
...
RegisterNUICallback('close', function(data, cb)
    TriggerEvent('charactercreator:close', data['save'])
end)
