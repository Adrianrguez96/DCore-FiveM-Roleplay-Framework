...
$(document).ready(function() {
    window.addEventListener('message', function(event) {
        if (event.data.action == 'setVisible') {
            ...
            $('#' + event.data.tab).show()
            $.post('https://charactercreator/setCurrentTab', JSON.stringify({
                tab: event.data.tab,
            }));
        }
...
function setView(event, view) {
    ...
    if (!wasActive) {
        $.post('https://charactercreator/setCameraView', JSON.stringify({
            view: view
        }));
        $.post('https://charactercreator/playSound', JSON.stringify({
...
$('#cameracontrol').on('mousemove', function(event) {
    if (moving == true) {
        ...
        lastOffsetX = offsetX;
        lastOffsetY = offsetY;
        $.post('https://charactercreator/updateCameraRotation', JSON.stringify({
            x: offsetX,
            y: offsetY,
        }));
    }
...
$('#cameracontrol').on('wheel', function(event) {
    ...
    $.post('https://charactercreator/updateCameraZoom', JSON.stringify({
        zoom: zoom,
    }));
});
...
function loadTabContent(tabName, charData, clothesData, identityData) {
    $.get('pages/' + tabName + '.html', function(data) {
        ...
        if (tabName == 'style') {
            ...
            let commoncolors = `
            <option value="0" selected="selected">Verde</option>
            <option value="1">Verde claro</option>
            <option value="2">Azul claro</option>
            <option value="3">Azul oscuro</option>
            <option value="4">Marron claro</option>
            <option value="5">Marron oscuro</option>
            <option value="6">Hazel</option>
            <option value="7">Gris oscuro</option>
            <option value="8">Gris claro</option>
            `
            let extracolors = `
...
function loadClothesTab(tab, clothesData, male) {
    ...
    topsoverlist.empty();
    topsoverlist.append($('<option data-component="11" data-drawable="15" data-texture="' + noover + '">Ninguno</option>')) // "None" option
    ...
    topsunderlist.empty();
    topsunderlist.append($('<option data-component="8" data-drawable="' + nounder + '" data-texture="0">Ninguno</option>')) // "None" option
    ...
    shoeslist.empty();
    shoeslist.append($('<option data-component="6" data-drawable="' + noshoes + '" data-texture="0">Ninguno</option>')) // "None" option
    ...
    */
    ...
    let nomask = 0;
    maskslist.empty();
    maskslist.append($('<option data-component="1" data-drawable="' + nomask + '" data-texture="0">Ninguno</option>')) // "None" option
    ...
    neckarmslist.empty();
    neckarmslist.append($('<option data-component="7" data-drawable="0" data-texture="0">Ninguno</option>')) // "None" option
    ...
    hatslist.empty();
    hatslist.append($('<option data-prop="0" data-drawable="-1" data-texture="0">Ninguno</option>')) // "None" option
    ...
    earslist.empty();
    earslist.append($('<option data-prop="2" data-drawable="-1" data-texture="0">Ninguno</option>')) // "None" option
    ...
    glasseslist.empty();
    glasseslist.append($('<option data-prop="1" data-drawable="-1" data-texture="0">Ninguno</option>')) // "None" option
    ...
    lefthandlist.empty();
    lefthandlist.append($('<option data-prop="6" data-drawable="-1" data-texture="0">Ninguno</option>')) // "None" option
    ...
    righthandlist.empty();
    righthandlist.append($('<option data-prop="7" data-drawable="-1" data-texture="0">Ninguno</option>')) // "None" option
...
$('.panelbottom button').on('click', function(evt) {
    evt.preventDefault();
    $.post('https://charactercreator/playSound', JSON.stringify({sound:'panelbuttonclick'}));
});
...
$('#main .menuclose').on('click', function(evt) {
    ...
    let action = accept ? 'guardar' : 'descartar';
    let message = '¿Esta seguro que usted quiere ' + action + ' sus cambios y salir?';
    let popupData = { 
        title: 'confirmación', 
        message: message
    };
...
function updateGender(value) {
    $.post('https://charactercreator/updateGender', JSON.stringify({
        value: value,
    }));
}

function updateHeadBlend(key, value) {
    $.post('https://charactercreator/updateHeadBlend', JSON.stringify({
        key: key,
        value: value,
    }));
}
...
function updateFaceFeature(key, value, index) {
    $.post('https://charactercreator/updateFaceFeature', JSON.stringify({
        key: key,
        value: value,
        index: index,
    }));
...
function updateEyeColor(value) {
    $.post('https://charactercreator/updateEyeColor', JSON.stringify({
        value: value,
    }));
}

function updateHairColor(key, value, highlight) {
    $.post('https://charactercreator/updateHairColor', JSON.stringify({
        key: key,
        value: value,
        highlight: highlight,
    }));
...
function updateHeadOverlay(key, keyPaired, value, index, isOpacity) {
    $.post('https://charactercreator/updateHeadOverlay', JSON.stringify({
        key: key,
...
function updateHeadOverlayExtra(key, keyPaired, value, index, keyExtra, valueExtra, indexExtra, isOpacity) {
    $.post('https://charactercreator/updateHeadOverlayExtra', JSON.stringify({
        key: key,
        keyPaired: keyPaired,
        value: value,
        index: index,
...
function closeWindow(save) {
    $.post('https://charactercreator/close', JSON.stringify({
        save: save
    }));
}

function updateOverlayColor(key, value, index, colortype) {
    $.post('https://charactercreator/updateOverlayColor', JSON.stringify({
        key: key,
        value: value,
        index: index,
        colortype: colortype,
...
function updateComponent(drawable, dvalue, texture, tvalue, index) {
    $.post('https://charactercreator/updateComponent', JSON.stringify({
        drawable: drawable,
        dvalue: dvalue,
        texture: texture,
        tvalue: tvalue,
...
function updateApparelComponent(drwkey, drwval, texkey, texval, cmpid) {
    $.post('https://charactercreator/updateApparelComponent', JSON.stringify({
        drwkey: drwkey,
        drwval: drwval,
        texkey: texkey,
        texval: texval,
...
function updateApparelProp(drwkey, drwval, texkey, texval, propid) {
    $.post('https://charactercreator/updateApparelProp', JSON.stringify({
        drwkey: drwkey,
        drwval: drwval,
        texkey: texkey,
        texval: texval,
...
$(document).on('click', 'input:radio[name=sex]', function(evt) {
    if(radioChecked == false)
    {
        let popupData = {
            title: 'Confirmación', 
            message: 'Cambiar el sexo puede resetear la customización que se estaba haciendo ¿Quiere realizar esta acción?'
        };
...
$(document).on('change', 'select.facepaintoverlay', function(evt) {
    ...
    if (prevtype != type) {
        $.post('https://charactercreator/syncFacepaintOpacity', JSON.stringify({
            prevtype: prevtype,
            currenttype: type,
        }));
        ...
        $.post('https://charactercreator/clearMakeup', JSON.stringify({
            clearopacity: false,
            clearblusher: true,
        }));
    }
...
$(document).on('change', 'select.makeuptype', function(evt) {
    ...
    $.post('https://charactercreator/updateMakeupType', JSON.stringify({type:value}));
});
